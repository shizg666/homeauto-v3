...
public class TransportConnection implements Disposable {
    ...
    public  Mono<Void> sendMessage(boolean isDup, MqttQoS qoS, boolean isRetain, String topic, byte[] message){
        return this.write(com.landleaf.homeauto.mqtt.api.MqttMessageApi.buildPub(isDup,qoS,isRetain,1,topic, Unpooled.wrappedBuffer(message)));
    }
    ...
    public   Mono<Void> sendMessageRetry(boolean isDup, MqttQoS qoS, boolean isRetain, String topic, byte[] message){
        ...
        this.addDisposable(id, Mono.fromRunnable(() ->
                this.write(com.landleaf.homeauto.mqtt.api.MqttMessageApi.buildPub(isDup,qoS,isRetain,id,topic, Unpooled.wrappedBuffer(message))).subscribe())
                .delaySubscription(Duration.ofSeconds(10)).repeat().subscribe()); // retry
        MqttPublishMessage publishMessage = com.landleaf.homeauto.mqtt.api.MqttMessageApi.buildPub(isDup,qoS,isRetain,id,topic, Unpooled.wrappedBuffer(message)); // pub
