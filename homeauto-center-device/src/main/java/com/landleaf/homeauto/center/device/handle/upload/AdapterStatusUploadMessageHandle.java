...
public class AdapterStatusUploadMessageHandle implements Observer {
    ...
    public void dealUploadStatus(AdapterDeviceStatusUploadDTO uploadDTO) {
        ...
        for (ScreenDeviceAttributeDTO dto : items) {
            ...
            valueDTOS.clear();
            redisBOList.clear();
            deviceStatusBOList.clear();
            TemplateDeviceDO deviceDO=  familyService.getFamilyDeviceByAttrCode(uploadDTO.getFamilyCode(),dto.getCode());
            String deviceCode = deviceDO.getCode();
//            HomeAutoProduct autoProduct= familyService.getFamilyDeviceProduct(uploadDTO.getFamilyId(),deviceCode);
            HomeAutoProduct autoProduct= iHomeAutoProductService.getById(deviceDO.getProductId());
            String productCode = autoProduct.getCode();
            try {
                /*******************************处理故障**********************************************/
                AttributeErrorDTO errorDTO = judgeFaultData(dto,productCode,deviceDO.getId());

                if (errorDTO == null) {
                    /****是否是glc特别需要存储的状态数据**********************/
                    if(StringUtils.equals(dto.getCode(),GLC_WIND_STATUS)){
                        log.info("新风特殊状态值,需要另存......");
                        filterSpecialGlcWindStatus(uploadDTO,dto,productCode,deviceCode);
                        continue;
                    }
                    // 添加到redis存储最新状态
                    // 添加到数据库状态存储
                    // 推送到websocket
                    redisBOList.add(generateRedisStoreData(uploadDTO, dto,deviceCode));
                    deviceStatusBOList.add(generateDBStoreData(uploadDTO, dto,productCode,deviceCode));
                    pushItems.add(dto);
                } else {
                    ...
                    if (type.intValue() == AttributeErrorTypeEnum.ERROR_CODE.getType().intValue()) {
                        havcDTOS.addAll(generateHavFaultData(errorDTO, uploadDTO, productCode, realestateId, projectId, dto,deviceCode));
                    } else if (type.intValue() == AttributeErrorTypeEnum.COMMUNICATE.getType()) {
                        //如果通讯故障为1，则入库
                        if (dto.getValue().equals(ErrorConstant.LINK_CODE_ERROR)) {
                            linkDTOS.add(generateLinkFaultData(uploadDTO, productCode, realestateId, projectId, dto,deviceCode));
                        }
                        ...
                        if (FaultValueUtils.isValueError(current, min, max)) {
                            valueDTOS.add(generateValueFaultData(current, min, max, uploadDTO, productCode, realestateId, projectId, dto,deviceCode));
                        }
//                        } else {
//                            //如果数值正常则进行正常处理
...
//                        }
                ...
                pushWebsocketData(pushItems, uploadDTO,deviceCode);
    ...
    private void filterSpecialGlcWindStatus(AdapterDeviceStatusUploadDTO uploadDTO, ScreenDeviceAttributeDTO dto,
         ...
         * 先不管它，直接存储
         */
        homeAutoGlcWindStatusService.saveRecord(uploadDTO.getFamilyId(),deviceCode,productCode,
...
//            iFamilyDeviceStatusService.insertBatchDeviceStatus(deviceStatusBOList);
     ...
     * @return
     */
    private HomeAutoFaultDeviceValueDTO generateValueFaultData(String current,
                                                               String min,
                                                               String max,
                                                               AdapterDeviceStatusUploadDTO uploadDTO,
                                                               String productCode,
        ...
        valueDTO.setDeviceCode(deviceCode);
     ...
     * @return
     */
    private HomeAutoFaultDeviceLinkDTO generateLinkFaultData(AdapterDeviceStatusUploadDTO uploadDTO,
                                                             String productCode,
                                                             String realestateId,
                                                             String projectId,
                                                             ScreenDeviceAttributeDTO dto,
        ...
        linkDTO.setDeviceCode(deviceCode);
     ...
     * @param dto
     */
    private List<HomeAutoFaultDeviceHavcDTO> generateHavFaultData(AttributeErrorDTO errorDTO,
                                                                  AdapterDeviceStatusUploadDTO uploadDTO,
                                                                  String productCode,
                                                                  String realestateId,
                                                                  String projectId,
...
//        List<String> stringList = errorDTO.getDesc();
        ...
        for (int i = 0; i < chars.length; i++) {
            if (chars[i] == '1') {
                ...
                havcDTO.setDeviceCode(deviceCode);
     ...
     * @return
     */
    private DeviceStatusBO generateDBStoreData(AdapterDeviceStatusUploadDTO uploadDTO,
                                               ScreenDeviceAttributeDTO dto,
                                               String productCode,
                                               String deviceCode) {

        DeviceStatusBO deviceStatusBO = new DeviceStatusBO();
        deviceStatusBO.setDeviceCode(deviceCode);
        ...
        deviceStatusBO.setCategoryCode(iHomeAutoProductService.getCategoryByProductCode(productCode).getCategoryCode());
     ...
     * 判断是否故障数据
     */
    private AttributeErrorDTO judgeFaultData(ScreenDeviceAttributeDTO dto, String productCode, String deviceId) {
...
//        request.setDeviceCode(deviceId);
