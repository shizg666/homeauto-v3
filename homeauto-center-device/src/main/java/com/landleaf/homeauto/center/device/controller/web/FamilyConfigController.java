...
public class FamilyConfigController extends BaseController {
    ...
    @ApiOperation(value = "同步家庭配置数据", notes = "")
    @ApiImplicitParam(name = CommonConst.AUTHORIZATION, value = "访问凭据", paramType = "header",required = true)
    @GetMapping("sync/{familyId}")
    public Response syncFamilyConfig(@PathVariable("familyId") String familyId){
        iHomeAutoFamilyService.syncFamilyConfig(familyId);
        return returnSuccess();
    }
