...
public class AuthJwtAccessTokenConverter implements TokenEnhancer, AccessTokenConverter, InitializingBean {
	...
	private static final Log logger = LogFactory.getLog(AuthJwtAccessTokenConverter.class);
	...
	public OAuth2AccessToken extractAccessToken(String value, Map<String, ?> map) {
		OAuth2AccessToken oAuth2AccessToken = tokenConverter.extractAccessToken(value, map);
		/**
		 * 增加自定义逻辑
		 */
	...
	public OAuth2AccessToken enhance(OAuth2AccessToken accessToken, OAuth2Authentication authentication) {
		...
		if (authentication.getPrincipal() instanceof HomeAutoUserDetails) {
			...
			HomeAutoSimpleUserDetails simpleUserDetails = new HomeAutoSimpleUserDetails();
			BeanUtils.copyProperties(baseUser,simpleUserDetails);
			// 将用户信息添加到token额外信息中
			info.put(CommonConst.TOKEN_ADDITION_MSG_KEY,JSON.toJSONString(simpleUserDetails));
		}
