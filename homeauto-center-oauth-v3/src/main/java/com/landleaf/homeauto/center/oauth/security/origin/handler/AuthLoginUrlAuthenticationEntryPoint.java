...
/**
 * @ClassName AuthLoginUrlAuthenticationEntryPoint   在securityConfig中加载
 * @Description: 自定义未登录返回json串而不是跳转到登录页   照着源码重写，唯一变化是修改失败后跳转，改为返回json数据 格式需统一 TODO
 * @Author wyl
 * @Date 2020/6/4
 * @Version V1.0
 **/
@Slf4j
public class AuthLoginUrlAuthenticationEntryPoint implements AuthenticationEntryPoint,
        InitializingBean {
    ...
    public void commence(HttpServletRequest request, HttpServletResponse response,
        ...
        returnResponse.setErrorMsg("未登陆");
        returnResponse.setSuccess(false);
        out.write(JSON.toJSONString(returnResponse));
        out.flush();
        out.close();
