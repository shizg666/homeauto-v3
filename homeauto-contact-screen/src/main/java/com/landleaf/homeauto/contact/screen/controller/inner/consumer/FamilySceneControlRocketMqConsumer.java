 ...
 * @author wenyilu
 */
@RocketMQConsumeService(topic = RocketMqConst.TOPIC_CENTER_ADAPTER_TO_CONTACT_SCREEN, tags = RocketMqConst.TAG_FAMILY_SCENE_SET)
@Slf4j
public class FamilySceneControlRocketMqConsumer extends AbstractMQMsgProcessor {
    ...
    protected MQConsumeResult consumeMessage(String tag, List<String> keys, MessageExt message) {
        ...
        ScreenMqttSceneControlDTO requestDto = null;
        try {
            ...
            if(requestDto.getOperateTime()+MESSAGE_EXPIRE<System.currentTimeMillis()){
                log.error("[下行==》场景控制 内部执行消息超过3分钟,已失效不再执行]");
                return result;
            }
        ...
        if (StringUtil.isEmpty(messageId) ||!redisUtils.getLock(RedisCacheConst.CONTACT_SCREEN_ROCKET_MQ_FROM_ADAPTER_SCENE_SET_SYNC_LOCK.concat(String.valueOf(messageId)),
                RedisCacheConst.COMMON_EXPIRE)) {
            log.error("[下行==》场景控制mq消息][消息编号]:{},重复消费或messageId为空",messageId);
            return result;
        }
        ...
        try {
            ...
            log.info("[下行==》场景控制mq消息]:消息类别:[{}],内部消息编号:[{}],外部消息编号:[{}],消息体:{}",
        ...
        redisUtils.del(RedisCacheConst.CONTACT_SCREEN_ROCKET_MQ_FROM_ADAPTER_SCENE_SET_SYNC_LOCK.concat(String.valueOf(messageId)));
