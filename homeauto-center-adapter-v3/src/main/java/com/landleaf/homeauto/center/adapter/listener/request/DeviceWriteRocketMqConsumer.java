...
public class DeviceWriteRocketMqConsumer extends AbstractMQMsgProcessor {
    ...
    protected MQConsumeResult consumeMessage(String tag, List<String> keys, MessageExt message) {
        ...
        AdapterDeviceControlDTO requestDto = null;
        try {
            String msgBody = new String(message.getBody(), "utf-8");

            requestDto = JSON.parseObject(msgBody, AdapterDeviceControlDTO.class);
            messageId=requestDto.getMessageId();
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }

        if (StringUtil.isEmpty(messageId) ||!redisUtils.getLock(RedisCacheConst.ADAPTER_ROCKET_MQ_FROM_APP_DEVICE_WRITE_SYNC_LOCK.concat(String.valueOf(messageId)),
        ...
        redisUtils.del(RedisCacheConst.ADAPTER_ROCKET_MQ_FROM_APP_DEVICE_WRITE_SYNC_LOCK.concat(String.valueOf(messageId)));
