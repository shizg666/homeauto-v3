 ...
 * @author zhanghongbin
 */
@RocketMQConsumeService(topic = RocketMqConst.TOPIC_CENTER_ADAPTER_TO_APP, tags = RocketMqConst.HVAC_POWER_UPLOAD)
@Slf4j
public class BridgeHVACPowerUploadRocketMqConsumer extends AbstractMQMsgProcessor {
    ...
    protected MQConsumeResult consumeMessage(String tag, List<String> keys, MessageExt message) {
        try {
            ...
            //转换为adapter功率上报结构体
            AdapterHVACPowerUploadDTO uploadDTO = JSON.parseObject(msgBody, AdapterHVACPowerUploadDTO.class);
