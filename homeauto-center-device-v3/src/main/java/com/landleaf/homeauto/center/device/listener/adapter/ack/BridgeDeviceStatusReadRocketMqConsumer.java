...
@RocketMQConsumeService(topic = RocketMqConst.TOPIC_CENTER_ADAPTER_TO_APP,
        tags = RocketMqConst.TAG_DEVICE_STATUS_READ)
@Slf4j
public class BridgeDeviceStatusReadRocketMqConsumer extends AbstractMQMsgProcessor {
    ...
    protected MQConsumeResult consumeMessage(String tag, List<String> keys, MessageExt message) {
        try {
            ...
            AdapterDeviceStatusReadAckDTO  ackDTO = JSON.parseObject(msgBody,AdapterDeviceStatusReadAckDTO.class);

            bridgeAckMessageService.dealMsg(ackDTO);
