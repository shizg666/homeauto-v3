...
@Slf4j
@Service
@Consumer(group = "WEB_SOCKET", topic = RocketMqConst.TOPIC_WEBSOCKET_TO_APPLETS)
public class WebSocketAppletsMessageHandler extends AbstractMessageHandler {
    ...
    public ConsumeConcurrentlyStatus consumeMessage(String[] keys, String message) {
        log.info("applets_消费消息:{}", JSON.toJSONString(message));
        try {
            MessageModel messageModel = JSON.parseObject(message, MessageModel.class);
            String familyId = messageModel.getFamilyId();
            String key = String.format("%s_%s",familyId,"applets");
            wsSendMsgExecutePool.execute(new MessageHandleRunnable(messageModel,key));
        } catch (Exception e) {
